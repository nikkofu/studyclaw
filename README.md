...
- 审核卡会显示学习素材来源标签：`手动录入 / 老师原文 / LLM 补全`，方便家长判断当前素材是否可靠、是否需要改
...
- 陪伴式持续监听已补强静音 / 无匹配场景的自动续听；在人工点“结束说话”前，会优先保持会话而不是因短暂空白直接退出
- 背诵 / 朗读工作台会把“真实停顿记录”和“标准原文逐句对照”分开呈现：上方保留真实开口节奏，下方再做标准完成度分析
- 背诵分析支持根据隐藏参考原文识别标题 / 作者；即使 transcript 前面带有短前导语，也会优先按参考原文的行句形态重整分段，再输出完成度、问题点和重背建议
...
- 任务解析、确认写入和任务板读取
- `/api/v1/tasks/parse`、草稿保存、`/api/v1/tasks/confirm` 和任务板读取会一起保留学习素材元数据与 `reference_source`
- `/api/v1/recitation/analyze` 提供背诵 / 朗读对照分析，支持规则兜底和 LLM 增强；规则兜底已补强前段标题/作者识别和参考行对齐
- 任务状态同步和自动积分
- 积分流水 / 余额
