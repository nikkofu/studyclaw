...
本文档描述 `2026-03-14` 的 `v0.3.4` 交付基线下的真实运行方式。当前正式运行形态是 `Go API + React Parent Web + Flutter Pad` 三端协同，不再需要额外的 Python 后端进程。
...
- `v0.3.4` 已支持短口令、长段朗读 / 背诵和陪伴式持续监听三种语音场景。
- `v0.3.4` 已修复 Web/STT 场景下“开始说话”后的启动判定与 `done / notListening` 收尾逻辑，并补上背诵分析主链。
