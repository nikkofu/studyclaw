...
> `2026-03-16` 状态说明：当前仓库已完成 `v0.4.0` 的本地收口验证，正式 GitHub release 基线可从 `v0.3.5` 切换到 `v0.4.0`。在真正执行远端动作前，仍应先复核本次 release scope、版本文件和发布资产是否只包含 `v0.4.0` 应纳入的内容。

本文档定义 `v0.4.0` 进入 GitHub 正式同步前的最小操作顺序。目标不是“把所有改动都推上去”，而是“只把本次交付范围内的源码、文档和版本文件推上去”。
