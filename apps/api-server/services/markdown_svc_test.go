...
func TestSaveTaskToMDAndGetTasksFromMD(t *testing.T) {
	t.Setenv("STUDYCLAW_DATA_DIR", t.TempDir())

	familyID := uint(101)
	userID := uint(202)

	if err := SaveTaskToMD(familyID, userID, "数学", "口算 30 题"); err != nil {
		t.Fatalf("SaveTaskToMD returned error: %v", err)
	}

	tasks, err := GetTasksFromMD(familyID, userID, time.Now())
	if err != nil {
		t.Fatalf("GetTasksFromMD returned error: %v", err)
	}
...
func TestGetTasksFromMDParsesCompletedCheckboxes(t *testing.T) {
	tempDir := t.TempDir()
...
func TestUpdateTaskCompletionFlows(t *testing.T) {
	...
	if err := SaveTaskWithGroupToMDAtDate(familyID, userID, "数学", "校本作业", "校本P14～15", date); err != nil {
		t.Fatalf("SaveTaskWithGroupToMDAtDate returned error: %v", err)
	}
	if err := SaveTaskWithGroupToMDAtDate(familyID, userID, "数学", "练习册", "练习册P12～13", date); err != nil {
		t.Fatalf("SaveTaskWithGroupToMDAtDate returned error: %v", err)
	}
	if err := SaveTaskWithGroupToMDAtDate(familyID, userID, "英语", "预习M1U2", "书本上标注好“黄页”出现单词的音标", date); err != nil {
		t.Fatalf("SaveTaskWithGroupToMDAtDate returned error: %v", err)
	}
	if err := SaveTaskWithGroupToMDAtDate(familyID, userID, "英语", "预习M1U2", "沪学习听录音跟读", date); err != nil {
		t.Fatalf("SaveTaskWithGroupToMDAtDate returned error: %v", err)
	}

	tasks, updatedCount, err := UpdateTaskCompletionByID(familyID, userID, date, 1, true)
	if err != nil {
	...
	tasks, updatedCount, err = UpdateTaskCompletionBySubject(familyID, userID, date, "数学", true)
	if err != nil {
	...
	tasks, updatedCount, err = UpdateTaskCompletionByHomeworkGroup(familyID, userID, date, "英语", "预习M1U2", true)
	if err != nil {
	...
	tasks, updatedCount, err = UpdateAllTasksCompletion(familyID, userID, date, true)
	if err != nil {
